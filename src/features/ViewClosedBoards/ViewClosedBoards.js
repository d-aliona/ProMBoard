...
const ViewClosedBoards = ({setShowViewClosedBoards}) => {
    ...
    const ref = useOutsideClick(() => {setShowViewClosedBoards(false)})
    
    useEffect(() => {
        const data = [...boards]
        const changedData = data.filter(el => el.owner === user.id && el.statusOpened === false)
    ...
    return (
        <>
            <div className={style.window}>
                <div className={style.boardForm} style={{maxWidth:'600px'}} ref={ref}>
                    <div className={style.title}>
                        <span className={style.titleName}>Closed boards</span>
                        <CloseButton onClick={() => {setShowViewClosedBoards(false)}} />
                    </div>
                    <Line />
                    <div className={styles.scrollbar}>
                        {closedBoards.length > 0
                            ? closedBoards.map((board) => {
                                return <OneClosedBoard 
                                            key={board.id}
                                            currentBoard={board}
                                        />
                            })
