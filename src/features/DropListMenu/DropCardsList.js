...
const DropCardsList = ({list, lists, cardsOnCurList, listsCardsToRender, setMessageMoveAllCards, setShowMenu}) => {
        ...
        cardsOnCurList.forEach(async(el, index) => {
            const docRef = doc(db, 'cards', el.id)
                  ...
                  await updateDoc(docRef, {
                    listID: selListID,
                    position: listsCardsToRender.find(it => it.list.id === selListID).cards.length + 1 + index,
                  })
        })
