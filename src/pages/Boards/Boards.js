...
const Boards = () => {
  ...
  useEffect(() => {
    const persBoardsArr =
      tempArr.filter((board) => {
        if (searchBoard !== '') {
          if (board.boardTitle.toLowerCase().includes(searchBoard.toLowerCase())) {
            return board
          }
        } else {
          return board
    ...
    const guestBoardsArr =
      guestBoards.filter((board) => {
        if (searchBoard !== '') {
          if (board.boardTitle.toLowerCase().includes(searchBoard.toLowerCase())) {
            return board
          }
    ...
    <>
      ...
      <div className={style.boardsGroup}>
        <div style={{display:'flex'}}>
          ...
          <div className={style.statistics}>
            Showing <b>{personalBoardsList.length}</b> of <b>{boards.length}</b> boards
          </div>
        </div>
        <div className={style.boards} >
          {personalBoardsList 
            && personalBoardsList.map((board) => 
             <OneBoardOnBoards key={board.id} board={board} isGuestBoard={false}/>
          )}
          {boards.length === 0 &&
      ...
      <div className={style.boardsGroup}>
        <div style={{display:'flex'}}>
          ...
          <div className={style.statistics}>
            Showing <b>{guestBoardsList.length}</b> of <b>{guestBoards.length}</b> boards
          </div>
        </div>
        ...
        <div className={style.boards} style={{maxHeight:'20vh'}}>
          {guestBoardsList 
            && guestBoardsList.map((board) => 
              <OneBoardOnBoards key={board.id} board={board} isGuestBoard={true}/>
          )}
