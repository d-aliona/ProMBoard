...
const Members = () => {
    ...
    return (
        <>
            ...
            <div className={styles.scrollbar} style={{maxHeight: `calc(100vh - 180px)`}}>
                {membersIDs &&
                    membersIDs.map((id) => {
                        const member = users.find(el => el.id === id)
                        let memberToDisplay = false
                        if (searchMem !== '') {
                            if (((member.firstName.toLowerCase() + member.lastName.toLowerCase()).includes(searchMem.toLowerCase())
                        ...
                        const memberBoards = memberToDisplay 
                            ? member?.guestBoards.filter(el => boards.filter(board => board.id === el)[0]?.owner === user.id)
                            : null
                            ...
                            <div className={style.boardsGroup} key={id}>
                                <div style={{ display: 'flex'}}>
                                    <h2 className={style.boardgroupTitle}>
                                        ...
                                        <span style={{color:'#666', fontSize:'14px',marginLeft:'20px'}}> {member.email}</span>
                                    </h2>
                                ...
                                <div style={{margin: '10px 40px 18px'}}>
                                    {memberBoards &&
                                        memberBoards.map((boardID) => {
                                            ...
                                            return (
                                                <div key={boardID}>
                                                    <div className={style.boardItemOnMembers}
                                                        onClick={() => navigate('/auth/board/' + boardID)}>
                                                        <div className={style.colorBoard} style={{backgroundColor: `${curBoard.boardColor}`}}></div>
